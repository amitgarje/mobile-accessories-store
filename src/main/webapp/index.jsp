    ...
    <%@ taglib uri="jakarta.tags.core" prefix="c" %>
        ...
        <head>
            ...
            <style>
                ...
                .minimal-card { transition: transform 0.4s cubic-bezier(0.16,1,0.3,1), box-shadow 0.4s; }
                .minimal-card:hover { transform: translateY(-10px) scale(1.01); box-shadow: 0 24px 48px rgba(0,0,0,0.1), 0 0 0 2px var(--accent); }
                .minimal-card .img-wrapper { position: relative; }
                .minimal-card .img-wrapper::after { content: ''; position: absolute; inset: 0; background: linear-gradient(135deg, rgba(255,255,255,0.15) 0%, transparent 60%); opacity: 0; transition: opacity 0.4s; }
                .minimal-card:hover .img-wrapper::after { opacity: 1; }

                /* Button ripple */
                .ripple-btn { position: relative; overflow: hidden; }
                .ripple-circle { position: absolute; border-radius: 50%; transform: scale(0); background: rgba(255,255,255,0.4); animation: rippleEffect 0.5s linear; pointer-events: none; }
                @keyframes rippleEffect { to { transform: scale(4); opacity: 0; } }

                /* Hero gradient */
                .hero-carousel { background: linear-gradient(135deg, #f0f4ff 0%, #fdf6ff 50%, #fff7ed 100%); }
                ...
                @keyframes particleFloat { 0% { transform: translateY(100vh) scale(0); opacity: 0; } 10% { opacity: 0.3; } 90% { opacity: 0.15; } 100% { transform: translateY(-100px) scale(1.2); opacity: 0; } }

                /* Section heading animated underline */
                .section-header h2 { position: relative; display: inline-block; }
                .section-header h2::after { content: ''; position: absolute; bottom: -6px; left: 0; width: 0; height: 3px; background: linear-gradient(90deg, var(--accent), #6C63FF); border-radius: 2px; transition: width 1s cubic-bezier(0.16,1,0.3,1); }
                .section-header h2.animated::after { width: 60%; }

                /* Navbar link glow */
                .nav-item:hover { text-shadow: 0 0 12px rgba(204,153,102,0.4); }

                /* Cursor spotlight (desktop) */
                .cursor-spotlight { position: fixed; pointer-events: none; width: 300px; height: 300px; border-radius: 50%; background: radial-gradient(circle, rgba(204,153,102,0.07) 0%, transparent 70%); transform: translate(-50%, -50%); transition: left 0.08s, top 0.08s; z-index: 9999; mix-blend-mode: multiply; }

                /* Offer card hover */
                .offer-card { transition: transform 0.4s cubic-bezier(0.16,1,0.3,1), box-shadow 0.4s !important; }
                .offer-card:hover { transform: translateY(-8px) !important; box-shadow: 0 20px 40px rgba(204,153,102,0.2) !important; }
